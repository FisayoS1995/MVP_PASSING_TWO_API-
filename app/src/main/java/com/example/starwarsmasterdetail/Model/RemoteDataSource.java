...
public class RemoteDataSource implements DataSource {
    ...
    public void getAllFilms() {
        starWarsService.getAllFilms().enqueue(new Callback<FilmsResponse>() {
            @Override
            public void onResponse(Call<FilmsResponse> call, Response<FilmsResponse> response) {
                listener.onFilmsRetrieve(response.body());
            }
    ...
    public void getFilmData(int planetNumber) {
        starWarsService.getPlanetDetails(planetNumber).enqueue(new Callback<PlanetResponse>() {
            @Override
            public void onResponse(Call<PlanetResponse> call, Response<PlanetResponse> response) {
                if (response.isSuccessful()) {
                    listener.onPlanetDetailRetreieved(response.body());
                }
